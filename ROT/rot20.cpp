...
string encrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] >= UPPER_A && message[i] < UPPER_G)
                cipher += message[i] + 20;
            else if (message[i] >= UPPER_G && message[i] <= UPPER_Z)
                cipher += message[i] - 6;
...
string decrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] > UPPER_T && message[i] <= UPPER_Z)
                decipher += message[i] - 20;
            else if (message[i] >= UPPER_A && message[i] <= UPPER_T)

    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] >= UPPER_A && message[i] < UPPER_M)
                cipher += message[i] + 14;
            else if (message[i] >= UPPER_M && message[i] <= UPPER_Z)
                cipher += message[i] - 12;
            else if (message[i] >= LOWER_A && message[i] < LOWER_M)
                cipher += message[i] + 14;
            else if (message[i] >= LOWER_M && message[i] <= LOWER_Z)
                cipher += message[i] - 12;
...
string decrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] > UPPER_N && message[i] <= UPPER_Z)
                decipher += message[i] - 14;
            else if (message[i] >= UPPER_A && message[i] <= UPPER_N)
                decipher += message[i] + 12;
            else if (message[i] > LOWER_N && message[i] <= LOWER_Z)
                decipher += message[i] - 14;
            else if (message[i] >= LOWER_A && message[i] <= LOWER_N)
                decipher += message[i] + 12;
            else if(message[i] < UPPER_A || message[i] > LOWER_Z || (message[i] > UPPER_Z && message[i] < LOWER_A))
                decipher += message[i];

...
string encrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] >= UPPER_A && message[i] < UPPER_K)
                cipher += message[i] + 16;
            else if (message[i] >= UPPER_K && message[i] <= UPPER_Z)
                cipher += message[i] - 10;
...
string decrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] > UPPER_P && message[i] <= UPPER_Z)
                decipher += message[i] - 16;
            else if (message[i] >= UPPER_A && message[i] <= UPPER_P)
                decipher += message[i] + 10;

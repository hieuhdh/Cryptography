...
string encrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] >= UPPER_A && message[i] < UPPER_E)
                cipher += message[i] + 22;
            else if (message[i] >= UPPER_E && message[i] <= UPPER_Z)
                cipher += message[i] - 4;
...
string decrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] > UPPER_V && message[i] <= UPPER_Z)
                decipher += message[i] - 22;
            else if (message[i] >= UPPER_A && message[i] <= UPPER_V)

...
string encrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] >= UPPER_A && message[i] < UPPER_U)
                cipher += message[i] + 6;
            else if (message[i] >= UPPER_U && message[i] <= UPPER_Z)
                cipher += message[i] - 20;
...
string decrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] > UPPER_F && message[i] <= UPPER_Z)
                decipher += message[i] - 6;
            else if (message[i] >= UPPER_A && message[i] <= UPPER_F)
                decipher += message[i] + 20;

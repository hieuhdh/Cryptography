...
string encrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] >= UPPER_A && message[i] < UPPER_C)
                cipher += message[i] + 24;
            else if (message[i] >= UPPER_C && message[i] <= UPPER_Z)
                cipher += message[i] - 2;
...
string decrypt(string message){
    ...
    for(int i = 0; i < message.size(); i++){
        ...
        if (message[i] != 32){ // Check for duplicates between message[i] and backspace key
            ...
            if (message[i] > UPPER_X && message[i] <= UPPER_Z)
                decipher += message[i] - 24;
            else if (message[i] >= UPPER_A && message[i] <= UPPER_X)
